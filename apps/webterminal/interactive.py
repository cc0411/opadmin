...
from opadmin.settings import MEDIA_ROOT
import threading
import ast
import traceback
from .utils import get_redis_instance, mkdir_p, CustomeFloatEncoder
import struct
import select
import paramiko
import logging
...
def posix_shell(chan, channel, log_name=None, width=90, height=40, elementid=None):
    ...
    command = list()
    if elementid:
        logobj = AuditLog.objects.get(channel=elementid)
    else:
        logobj = AuditLog.objects.get(channel=channel)
    vim_flag = False
    vim_data = ''
    try:
        chan.settimeout(0.0)
...
class SshTerminalThread(threading.Thread):
    ...
    def run(self):
        ...
        command = list()
        if self.elementid:
            logobj = AuditLog.objects.get(channel=self.elementid)
        else:
            logobj = AuditLog.objects.get(channel=self.message.reply_channel.name)
        while (not self._stop_event.is_set()):
            text = self.queue.get_message()
            if text:
                # deserialize data
                if isinstance(text['data'], (str, unicode,basestring, bytes)):
