...
class PlayBookResultsCollector(CallbackBase):
    ...
    def v2_playbook_on_task_start(self, task, is_conditional):
        msg = '<pre>\nTASK [{}] {}\n</pre>'.format(task.get_name(), '*' * 100)
        self.send_save(msg)

    def v2_runner_on_ok(self, result, *args, **kwargs):
        if result.is_changed():
            data = '<pre>[{}]=> changed\n</pre>'.format(result._host.name)
        else:
            data = '<pre>[{}]=> ok\n</pre>'.format(result._host.name)
        self.send_save(data)
    ...
    def v2_runner_on_failed(self, result, *args, **kwargs):
        ...
        data = '<pre>[{}]=> {}: {}\n</pre>'.format(result._host.name, 'failed',
    ...
    def v2_runner_on_unreachable(self, result):
        if 'changed' in result._result:
            del result._result['changed']
        data = '<pre>[{}]=> {}: {}\n</pre>'.format(result._host.name, 'unreachable',
                                                                            self._dump_results(result._result))
        self.send_save(data)
    ...
    def v2_runner_on_skipped(self, result):
        if 'changed' in result._result:
            del result._result['changed']
        data = '<pre>[{}]=> {}: {}\n</pre>'.format(result._host.name, 'skipped',
                                                                            self._dump_results(result._result))
        self.send_save(data)
    ...
    def v2_playbook_on_stats(self, stats):
        ...
        for h in hosts:
            s = stats.summarize(h)
            msg = '<pre>{} : ok={} changed={} unreachable={} failed={} skipped={}\n</pre>'.format(
...
class ANSRunner(object):
    ...
    def run_module(self, host_list, module_name, module_args, deploy=False, send_msg=True):
        ...
        """
        self.callback =  ModuleResultsCollector(sock=self.sock)
