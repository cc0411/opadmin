...
class ElfinderConnectorView(LoginRequiredMixin,View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if kwargs['optionset'] == 'sftp':
            server_object = get_object_or_404(
                Hosts, id=kwargs['start_path'])
            optinon_sets = self.get_optionset(**kwargs)
            optinon_sets['roots'][u_id][0]['alias'] = '{0}-{1}'.format(
                server_object.hostname, server_object.nip)
            key_label = "%s::%s" % (
                server_object.nip, server_object.user.username)
            if server_object.user.auth_method == 'ssh-password':
                optinon_sets['roots'][u_id][0]['storageKwArgs'] = {'host': server_object.nip,
                                                                   'params': {'port': server_object.user.ssh_port,
                                                                              'username': server_object.user.username,
                                                                              'password': c().decrypt(server_object.user.password),
                                                                              'timeout': 30},
                                                                   'root_path': '/', 'interactive': False,
                                                                   'key_label': key_label}
            else:
                private_key = StringIO(server_object.user.key)
                key = server_object.user.key
                if 'RSA' in key:
    ...
    def post(self, request, *args, **kwargs):
        ...
        if kwargs['optionset'] == 'sftp':
            ...
            optinon_sets = self.get_optionset(**kwargs)
            optinon_sets['roots'][u_id][0]['alias'] = '{0}-{1}'.format(
                server_object.hostname, server_object.wip)
            key_label = "%s::%s" % (
                server_object.nip, server_object.user.username)
            if server_object.user.auth_method == 'ssh-password':
                optinon_sets['roots'][u_id][0]['storageKwArgs'] = {'host': server_object.nip,
